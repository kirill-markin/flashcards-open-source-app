...
Two [alarms](../infra/aws/lib/monitoring.ts) route here. `GeoLiteCountryDatabaseStaleAlarm` fires while lookups still work, once the object being served was uploaded four days ago, and means four consecutive daily refreshes failed to land; the seven-day lifecycle expiry then deletes that object about three days later, so this alarm is the one that leaves time to act. `GeoLiteCountryLookupFailureAlarm` fires once lookups throw, on either the consent route or the credential-free collector, and by then every visitor worldwide is being treated as consent-required and every credential-free analytics event that carries a country is being refused with a 500 rather than stored without one, while the events [exempt from carrying one](anonymous-client-analytics.md) keep landing with a 200.
...
The bucket blocks public access, requires TLS, uses SSE-S3 encryption and has no version history. Replacements remove the superseded object immediately; seven-day lifecycle expiry removes abandoned objects when updates stop. CI temporary files are removed on exit and never uploaded as artifacts or caches. Lambda deletes its temporary MMDB immediately after opening it and retains only the reader in memory. Freshness enforcement uses the database build date, never S3 upload time; the staleness alarm measures the S3 upload instead, because that is what the lifecycle expiry counts against. The 30-day maximum is enforced on every lookup, including warm instances. See [MaxMind update requirements](https://dev.maxmind.com/geoip/geolite2-free-geolocation-data/#automate-database-downloads-and-updates).
