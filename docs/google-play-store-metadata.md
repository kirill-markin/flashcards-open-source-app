...
This file holds hand-written Play listing copy: one section per language, each written for its own market rather than translated from the English default. Most sections match a language the apps themselves ship, namely English, Arabic, Chinese (Simplified), French, German, Hindi, Japanese, Portuguese (Brazil), Russian, and Spanish for both Mexico and Spain; the `Spanish (Latin America) - es-419` section below is the Play code for the `es-MX` product locale. That overlap is incidental rather than a rule: `Spanish (United States) - es-US` has no matching product locale, and listing copy for a market may be written before the in-app translation for that language exists.
