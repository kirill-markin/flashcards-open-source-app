...
- platform is read off the event row and never derived; the buckets are `web` / `android` / `ios` / `agent` / `unattributed` and are always split, never summed, so agent-API activity cannot read as a person on the site
- `review_answered` is server-derived and carries no platform by design, so every review row lands in `unattributed`, meaning "not a device fact"; a real per-platform split of people belongs to the `app_opened` series
