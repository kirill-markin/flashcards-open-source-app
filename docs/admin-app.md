...
Every charted surface also drops the actors listed in `analytics.excluded_actors` while their exclusion is active, meaning `restored_at IS NULL`: it is one shared list of actors no human produced, defined with its restore rules in `db/migrations/0140_analytics_excluded_actors.sql`, applied to every event of such an actor that a charted number counts rather than to one chart, and restated in the filter option lists so a value only an excluded actor produced is never offered; the `catalog-installation` funnel is the one surface that can apply it only partly, because a journey is anonymous and names an actor only through a matching server install or a matching signed-in install start, so a journey that neither completed a server install nor started an install while signed in names no actor and stays counted. Those two bridges also carry the older `%@example.com` and active-admin exclusions, which until now reached this funnel only through a server install, so the funnel and the no-click diagnostic drop the moment this lands and before any actor is listed at all: a journey whose signed-in install start belongs to a test address or an active admin is dropped even when no server install matches it.
...
- new versus returning is the installer's first `app_opened` day, which is `daily-active-users`' cohort definition rather than one of this section's own, rebuilt as a CTE over the same event so the two sections cannot disagree about which day a person was new on
- that CTE only covers installers with an `app_opened` day inside the selected range, so an installer without one is in neither cohort and is kept only while both cohorts are selected, rather than being guessed into `new` or `returning`
- every field of the shared filter bar applies, all of them in SQL, so any filter change reloads the section
...
- failure totals count distinct attempts per observed stage/reason, so they are not inferred abandonments and are not mutually exclusive; the report separately shows selected-range landings without an earlier selected-range click, server installs carrying no journey at all, and click attempts whose seven-day window is still maturing
- post-install engagement is reported only for attempts that completed a server install, which is the one row naming the person, and is split into a new-account and an existing-account column, each measured against its own installs; the steps are one review, twenty reviews, and twenty reviews plus a review on a later UTC day than the install, nested so no step can exceed the one above it. The reviews are that person's `review_answered` rows anywhere in the product rather than in the installed deck, because the event carries no deck or card identity; they are counted from the install to the same seven-day bound on the click, so a late install has less of that window; new means the actor produced no event at all before the click, read over that actor's whole history
- a server install whose `occurred_at` lands on a selected UTC day and carries no `install_journey_id` belongs to no journey and is counted on its own line, outside every funnel denominator; a confirm may legitimately omit the journey, so this is evidence rather than a defect. The line counts only installs carrying no `install_journey_id`, which makes it a lower bound on what the funnel cannot hold rather than the whole of it: an install that does carry a journey is equally unheld when that journey's click fell outside the selected cohort dates, was dropped by a placement, source, device-category or browser-language selection, or has a broken landed/preview/started/installed chain. That line reads the installed deck, the client platform and the actor exclusions off the install row itself, and a server install carries no platform, so selecting any device platform empties it
- the delisted `test` deck is excluded when an install-start fact identifies it; `@example.com`, active-admin and analytics-exclusion-list journeys are excluded when either a matching server install or a matching signed-in `catalog_deck_install_started` identifies the actor. The signed-in install start is new for the first two, which previously needed the server install, so such a journey now drops even when its install failed or its confirm omitted `installJourneyId`. The public collector stores no user identity or identity link, so an attempt that neither completed a server install nor started an install while signed in cannot always be classified or excluded
...
- the Audience cohort is the distinct resolved actors with an `app_opened` event in range, excluding active admins, example.com accounts and actors on the analytics exclusion list, and counting merged guests only once
