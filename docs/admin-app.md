...
- all seven shared filters apply, every one of them in SQL, so any filter change reloads the section
...
- pick one connection country and one app UI language in General; confirm every chart and the Audience denominator narrow to the people carrying that country sample and that locale, that the two popups say in plain words that country history is kept for 90 days only and that a person with no retained sample or no recorded locale matches nothing, and that both selections survive a reload through the URL
- date, user, new/returning cohort, platform, minimum-event-count, connection country, and app UI language filters open as popups from the one shared filter row, each naming in the popup what it filters and how it is counted; the date popup opens a two-month UTC calendar whose second click applies the range, and the threshold popup takes one count per event type, all of which a user must clear; a count it cannot hold, such as `1.5`, stays in its input and is named as not applied, leaving the chips and the button showing the threshold that is
