...
- a `review_answered` row carries the platform the backend resolved from the replica that recorded the review, and migration `0122` filled the same value on the reconstructed history; a value appears only for a `client_installation` replica on `ios`, `android`, or `web`, while a machine-API, an AI-chat, and a seed/reset replica all leave the column NULL, as does a review whose replica row is gone
