...
- all five shared filters apply, every one of them in SQL, so any filter change reloads the section
...
- the user filter list and the chart colour domains come from their own range-scoped query, so a filter that empties a chart never removes a user or a deck from the options or changes anyone's colour
