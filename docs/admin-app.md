...
- all four shared filters apply, every one of them in SQL, so any filter change reloads the section
...
- dashboard filters can narrow date range, user, new/returning cohort, and platform; all four are applied in SQL and reach every chart, including the community charts, where a cohort or platform filter keeps community rows only for users that still have review events in range, and the user filter list also offers users with community activity but no review events in range; Reset all returns date range to the same last-30-days default and clears every other filter
...
- a filter click shows `Updating` at once and leaves every selection control live, so a second click during a reload supersedes the first; only Apply, Reset time and Reset all are disabled while a reload runs
