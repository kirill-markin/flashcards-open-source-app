...
The audience mode is not a filter-bar field, so `Reset all` does not clear it: a reset returns the default range and every filter, and the funnels keep the audience they were reading. Removing the `audience` parameter from the URL, or picking `With anonymous ID`, is what returns it.
...
Every charted surface and every filter option list drops the same people, through the single rule written in [`buildExcludedActorSqlLines`](../apps/admin/src/filters/filterSql.ts): an `@example.com` test account, anyone who has ever held an `auth.admin_users` row whether or not it was revoked, and an actor listed in `analytics.excluded_actors` and not restored, meaning `restored_at IS NULL` (`db/migrations/0140_analytics_excluded_actors.sql` defines that list and its restore rules). The rule is applied to every event of such a person that a charted number counts, and restated in the option lists so a value only an excluded person produced is never offered. The public `GET /v1/global/snapshot` applies the same rule to its own identity column ([global-metrics.md](global-metrics.md)). Every surface applies it on the row's own `actor_id`, the `catalog-installation` funnel included, because the shared visitor identity names somebody from the first deck page view; a visitor who never signed in resolves to their own browser id, which no account's address can name, so only the exclusion list itself can reach such a row. Almost every catalog install in production history is an admin install, so `catalog-deck-installs` is nearly empty by design.
...
- the Audience cohort is the distinct resolved actors with an `app_opened` event in range, excluding the people the one rule drops - admins, example.com accounts and actors on the analytics exclusion list - and the rows the credential-free collector wrote, and counting merged guests only once
