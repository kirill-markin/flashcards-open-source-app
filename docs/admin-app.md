...
- a `review_answered` row carries the platform the backend resolved from the replica that recorded the review, and migration `0122` filled the same value on the reconstructed history, `0123` on the live rows the producer wrote before it could resolve one; a device value appears only for a `client_installation` replica on `ios`, `android`, or `web` and a machine-API replica resolves to `agent`, while an AI-chat and a seed/reset replica both leave the column NULL, as does a review whose replica row is gone
