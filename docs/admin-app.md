...
New journeys stopped being recorded on 2026-09-21, so this funnel reports a closed period that only browsers still running the previous bundle, or holding a cached marketing-site page, still extend, a tail of days rather than release cycles, and it returns on the shared visitor identity once the domain move puts the site and the app under one registrable domain that identity can span. The events this reads, and what is left of the journey id, are in [catalog-install-funnel.md](catalog-install-funnel.md).
