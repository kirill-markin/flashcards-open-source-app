...
`Funnels` is separate from General and stacks its funnels in one fixed order, top to bottom: mobile first launch, home page to web app, deck page to install, blog article to web app. All four exist: `mobile-first-launch`, `home-to-web-app`, `catalog-installation` (the deck funnel) and `blog-to-web-app`; the order lives in the single [`funnelSections`](../apps/admin/src/reports/funnels/funnelSections.ts) list. Each funnel owns its query, parser, and display, reuses one step chart whose click-to-anchor step rides in the URL under its own parameter (`funnelFrom` for the deck funnel, `<funnelId>From` for the others: `mobileFrom`, `homeFrom` and `blogFrom`), and so can be added without changing General or introducing a generic reporting engine. The shared bar above them offers only the fields every funnel shares - date range, client platform, connection country and app interface language - and a field that belongs to one funnel sits in that funnel's own filter row, with the same URL parameter it has everywhere else. Each funnel also carries its own compact `Group by` field in its own filter row area under its heading, with its own dimension list and its own URL parameter `<funnelId>GroupBy`, omitted while it holds its `None` default; grouping one chart leaves the others alone. The field sits outside the chart because the chart is not drawn while a funnel loads or when nothing matches the filters, and a grouping has to stay clearable in both. Grouping is per person: the key is a property of the person over the selected dates, so the groups partition the funnel and sum back to it step by step, and every group is measured inside itself, from its own first step and from its own count at a selected step, so two groups of very different size compare as rates rather than as bar heights. A grouped chart draws the five largest groups, sums everything below them into one grey `Other`, and pins the people the dimension cannot place in their own `Unresolved` group, which is never folded and never takes one of the five. [`funnelGroupBy.ts`](../apps/admin/src/reports/funnels/funnelGroupBy.ts) owns the contract, the URL codec, the folding and the colours, [`FunnelGroupByPicker.tsx`](../apps/admin/src/reports/funnels/FunnelGroupByPicker.tsx) is the field, and each funnel declares its own dimensions in its own query. Only `catalog-installation` declares none so far, so it is the one funnel with no field.

Every funnel follows one rule, written beside the [`funnelSections`](../apps/admin/src/reports/funnels/funnelSections.ts) list: each step grows by at most one per person, and a person counts at a step only after reaching every earlier step. Each funnel counts entries from a fixed start date in its query, so no selection shows a partial first day: 2026-09-01 for mobile first launch ([`mobileFirstLaunchFunnelStartDate`](../apps/admin/src/reports/mobileFirstLaunchFunnel/query.ts)), and 2026-09-23 for home page, blog article ([`siteEntryFunnelStartDate`](../apps/admin/src/reports/siteEntryFunnel/query.ts)) and deck page ([`catalogInstallFunnelStartDate`](../apps/admin/src/reports/catalogInstallFunnel/query.ts)). A range that starts earlier shows a note, and a range holding entries still inside their seven-day window shows a warning instead of cutting them.

One control above all four funnels, [`FunnelAudienceModeControl`](../apps/admin/src/reports/funnels/FunnelAudienceModeControl.tsx), picks who they count. It is a single choice rather than a filter-bar field, it applies to every funnel at once so two funnels are always comparable, and it rides in the URL as `audience`, omitted while it holds its default. The three modes are declared in [`funnelAudienceModes`](../apps/admin/src/filters/analyticsFilters.ts) and expressed in SQL in [`funnelAudienceSql.ts`](../apps/admin/src/reports/funnels/funnelAudienceSql.ts):
...
- `all` adds a second cohort of cookieless visitors, keyed on `analytics.product_events_resolved.daily_visitor_hash` plus the UTC day, one person per pair, counted only from [`funnelDailyVisitorHashStartDate`](../apps/admin/src/reports/funnels/funnelAudienceSql.ts). They are drawn as a lighter segment inside the bars, the shares are measured on the total, and the split is written above the bar and in the chart's hidden table. They can reach the site steps alone - a page view, then the web app link click or an install click on a deck whose page they viewed - because every step below reads a trusted in-app row that a cookieless browser never sends; their segment ending there is construction rather than drop-off. The mobile funnel has no such people at all, so `all` equals the default there
...
- `Group by` offers two dimensions, `country` and `language`, read from the same per-actor fragments in [`filterSql.ts`](../apps/admin/src/filters/filterSql.ts), and under the same rules, as the mobile funnel's two of those names; the client platform is not offered, because the entry page view is always web. The key comes from a person's trusted events in range, so anyone who never signed in falls in `Unresolved`. The cookieless visitors of `All` are in `Unresolved` under either dimension too, keeping the lighter segment of the first two bars there: they carry no actor, so their country is unknowable, and their page view's own `ui_locale`, which the query does read, is deliberately not grouped on, so a language group is short by the cookieless people who carry that language rather than containing them; [`query.ts`](../apps/admin/src/reports/siteEntryFunnel/query.ts) declares both

Attribution contract for the `catalog-installation` funnel, shown as "Deck page to install":

- every step counts people, not rows: a person adds at most one to each step, and only after reaching every earlier step, so step one, "Viewed any deck page", is a person who viewed the page of any deck, and the click and everything below it must be on a deck whose page that person viewed. `countPeopleAtEachStep` in [`CatalogInstallFunnelSection.tsx`](../apps/admin/src/reports/catalogInstallFunnel/CatalogInstallFunnelSection.tsx) owns the reduction
