...
- one row is one person whose first trusted event anywhere, over every event name and all of history, is an iOS or Android `app_opened` on a selected UTC day. The one exception is a `card_created` at most 60 seconds before that open, the [demo card](demo-card.md) seeded during first launch; any earlier `card_created` or other event keeps the person out; the later steps are the review screen, an answer reveal, a first `review_answered`, twenty reviews, and twenty reviews plus one on a later UTC day than the first app open, each in order and within the deck funnel's seven-day window. First opens count only from the later of the selected start and the first UTC day on which iOS or Android had reported both the review-screen `screen_viewed` and `review_card_revealed`, and the report returns that effective start so the section can say when it cuts the range. [`query.ts`](../apps/admin/src/reports/mobileFirstLaunchFunnel/query.ts) owns the definitions
...
- the report returns the first UTC day the site reported an identified page view, so the section says why an earlier range is empty; the date range, the client platform (read off the entry page view, always web), the connection country and the app interface language all apply in SQL, the last two as on General; `@example.com`, active-admin and analytics-exclusion-list actors are excluded
...
- people count only from the later of the selected start and the first UTC day the site reported an identified deck page view, and the report returns that effective start, so a range before it shows a note rather than drop-off. Older, click-only history is deliberately not reachable here: the funnel starts at the page view
