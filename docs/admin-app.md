...
`Funnels` is separate from General and stacks its funnels in one fixed order, top to bottom: mobile first launch, home page to web app, deck page to install, blog article to web app. Two exist today, `mobile-first-launch` first and `catalog-installation`, the deck funnel, after it; the order lives in the single [`funnelSections`](../apps/admin/src/reports/funnels/funnelSections.ts) list. Each funnel owns its query, parser, and display, reuses one step chart whose click-to-anchor step rides in the URL under its own parameter (`funnelFrom` for the deck funnel, `<funnelId>From` for the others, such as `mobileFrom`), and so can be added without changing General or introducing a generic reporting engine. The shared bar above them offers only the fields every funnel shares - date range, client platform, connection country and app interface language - and a field that belongs to one funnel sits in that funnel's own filter row, with the same URL parameter it has everywhere else.
...
- General still contains every existing chart; Funnels opens separately with a shared bar of date range, client platform, connection country and app interface language, Mobile first launch sits first with no filter row of its own, and Catalog installation carries its deck and catalog-click filters in its own row
