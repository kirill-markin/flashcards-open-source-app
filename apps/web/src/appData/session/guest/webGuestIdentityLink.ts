...
import { isProductAnalyticsCollectionEnabled } from "../../../analytics/productAnalyticsCollection";
 ...
 *
 * Pass the analytics preferences sync started beside this call as well. It is what the gate inside
 * waits for.
 */
export function linkWebGuestIdentityInBackground(
  ...
  capturedIdentityGeneration: number,
  accountUserId: string,
  analyticsPreferencesSync: Promise<void>,
): void {
  if (guestSession === null) {
    return;
  }

  // The account's own answers have to reach this browser before the gate below reads them. The sync
  // is started unawaited on the line above this call and the account wins wherever both records
  // exist, so a gate run first would read the browser's pre-sync answers — and a browser holding
  // `granted`, or collection on, under an account holding the opposite would spend the guest
  // identity on a decision the account is about to overrule, permanently. The sync swallows its own
  // failures, so this settles either way, and it is a background task like the link itself: nothing
  // in the sign-in path waits on it.
  void analyticsPreferencesSync.then((): void => {
...
function startGuestIdentityLink(
  ...
  // The operator opt-out, a person who turned product analytics off, a refused consent banner, and
  // a banner still waiting to be answered all stop this before it spends the guest identity a
  ...
  // make for somebody who is not being measured — and it routinely runs before
  // `GET /v1/analytics/visitor` has even said whether this browser has to be asked, because that
  // call can pay a GeoLite download. Both switches outlive every local data wipe, so the visitor
  // who minted a guest under an earlier build and only then opted out still arrives here. On
  // everything but a refusal — the two opt-outs, and a banner still open — the envelope and its
  // stamp are left alone rather than dropped: nothing mints or republishes a web guest any more
  // (`webGuestSession.ts`), so the envelope sits inert, and keeping it is what lets the tail still
  // be linked on the load that answers the banner or turns analytics back on, as long as it
  // arrives before the next identity boundary. A refusal is the case where nothing is kept: it
  // drops the envelope outright as it retires every other identifier on the device
  // (`applyAnalyticsConsentDecline` in `deliveryRuntime.ts`), so a browser that refused reaches
  // this gate with nothing left to spend.
  if (
    isAnalyticsEnabledForCurrentRuntime() === false
    || isProductAnalyticsCollectionEnabled() === false
    || isAnalyticsIdentityConsented() === false
  ) {
    return;
  }
