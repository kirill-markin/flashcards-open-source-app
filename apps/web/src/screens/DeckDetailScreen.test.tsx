...
describe("DeckDetailScreen", () => {
  ...
  it("renders the virtual All cards deck as read-only and opens review for all cards", async () => {
    ...
    await act(async () => {
      root.render(
        <MemoryRouter initialEntries={["/settings/workspace/decks/all-cards"]}>
          <Routes>
            <Route path="/settings/workspace/decks/:deckId" element={<DeckDetailScreen />} />
  ...
  it("renders persisted deck actions and matching cards", async () => {
    ...
    await act(async () => {
      root.render(
        <MemoryRouter initialEntries={["/settings/workspace/decks/deck-1"]}>
          <Routes>
            <Route path="/settings/workspace/decks/:deckId" element={<DeckDetailScreen />} />
          </Routes>
        </MemoryRouter>,
  ...
  it("shows a not-found state for missing persisted decks", async () => {
    ...
    await act(async () => {
      root.render(
        <MemoryRouter initialEntries={["/settings/workspace/decks/missing"]}>
          <Routes>
            <Route path="/settings/workspace/decks/:deckId" element={<DeckDetailScreen />} />
          </Routes>
        </MemoryRouter>,
