...
// `GUEST_AI_LIMIT_REACHED` is the guest-only code `AI_LIMIT_REACHED` superseded; it cannot reach the
// web app, whose guest credential is refused on every authenticated surface, and it stays matched only
// so a deployed backend still raising it is handled the same way.
