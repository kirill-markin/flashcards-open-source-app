...
describe("ChatPanel send lifecycle", () => {
  ...
  it("reconciles run_terminal completion when the assistant materialization is still incomplete", async () => {
    getChatSnapshotMock
      .mockResolvedValueOnce(createChatSnapshot())
      .mockResolvedValueOnce(createChatSnapshot({
        sessionId: "session-1",
          ...
          messages: [
            ...
            },
            {
              ...
              content: [
                ...
                { type: "text", text: "You have 10 cards." },
              ],
              timestamp: 2,
              isError: false,
              isStopped: false,
    ...
    consumeChatLiveStreamMock.mockImplementation(async ({ onEvent }) => {
      onEvent({
        ...
        sequenceNumber: 1,
        streamEpoch: "epoch-1",
        text: "I'm checking your workspace card count now.",
        cursor: "cursor-1",
        itemId: "item-1",
      });
      onEvent({
    ...
    expect(getChatSnapshotMock).toHaveBeenCalledTimes(2);
    expect(getContainer().querySelector('[role="dialog"]')).toBeNull();
    expect(getContainer().textContent).toContain("You have 10 cards.");
