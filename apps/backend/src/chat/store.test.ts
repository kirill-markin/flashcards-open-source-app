...
test("touchChatSessionHeartbeatWithExecutor updates heartbeat and active run on an existing session", async () => {
  const executor: DatabaseExecutor = {
    async query<Row extends QueryResultRow>(
      text: string,
      params: ReadonlyArray<SqlValue>,
      ...
      if (text.includes("UPDATE ai.chat_sessions")) {
        ...
        assert.equal(params[2], "run-1");
        assert.equal(params[3], "2026-03-25T11:00:00.000Z");
        return makeQueryResult<Row>([{
