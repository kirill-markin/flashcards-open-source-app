...
test("buildLocalSystemInstructions includes strict tool-call rules, examples, and user context", () => {
  ...
  assert.match(instructions, /sql => \{"sql": "SELECT \* FROM cards WHERE due_at IS NULL OR due_at <= NOW\(\) ORDER BY due_at ASC, created_at DESC, card_id ASC LIMIT 20 OFFSET 0"\}/);
