import {
  ...
  getPublicApiBaseUrl,
  getPublicLegalLinks,
  type PublicAgentDocs,
} from "../shared/publicUrls";
import {
  GUIDE_TOPIC_DESCRIPTIONS,
  GUIDE_TOPICS,
  type GuideTopic,
...
type AgentDiscoveryEnvelope = Readonly<{
  ...
  data: Readonly<{
    ...
    surface: Readonly<{
      ...
      guideUrlTemplate: string;
      usageLimitsUrl: string;
      nextReviewCardUrl: string;
      revealAnswerUrl: string;
      ...
      catalogPackageVersionUrlTemplate: string;
...
/**
 * Rendered from `GUIDE_TOPIC_DESCRIPTIONS` rather than written out, so discovery cannot advertise a
 * topic that `GET /agent/guide/{topic}` rejects: adding, removing, or renaming a topic is a type
 * error at that map instead of stale prose here.
 */
const guideTopicClause = GUIDE_TOPICS.map(
  (topic) => `${topic} for ${GUIDE_TOPIC_DESCRIPTIONS[topic]}`,
).join("; ");

/** Typed so a rename of this one topic is a type error here too, not a stale name in the prose. */
const sqlDialectTopic: GuideTopic = "sql_dialect";
...
export function createAgentDiscoveryEnvelope(requestUrl: string): AgentDiscoveryEnvelope {
  ...
  const guideUrlTemplate = `${apiBaseUrl}/agent/guide/{topic}`;
  ...
  const catalogPackageVersionUrlTemplate = `${apiBaseUrl}/catalog/package-versions/{packageVersionId}`;
  ...
  const envelope: AgentDiscoveryEnvelope = {
    ...
    data: {
      ...
      capabilitiesAfterLogin: [
        "Review one card at a time, reveal its answer, and submit an idempotent FSRS rating through /agent/reviews/{next,reveal,submit}",
        "Load account context",
        "Select a workspace",
        "Inspect the SQL surface through runtime discovery and SQL introspection",
        ...
        "Fetch one reference guide at a time through GET /agent/guide/{topic} for the SQL dialect, card authoring, bulk authoring, and the review flow",
        "Read the plan tier, the limits resolved for it, and this month's AI consumption through GET /agent/usage-limits",
      ...
      surface: {
        ...
        guideUrlTemplate,
        usageLimitsUrl: `${apiBaseUrl}/agent/usage-limits`,
        nextReviewCardUrl: `${apiBaseUrl}/agent/reviews/next`,
        revealAnswerUrl: `${apiBaseUrl}/agent/reviews/reveal`,
        submitReviewUrl: `${apiBaseUrl}/agent/reviews/submit`,
        ...
        catalogPackageVersionUrlTemplate,
      ...
      mcp: {
        url: `${mcpBaseUrl}/mcp`,
        description:
          "Remote MCP server for AI clients that connect through custom connectors (for example Claude.ai or ChatGPT). Add the url as a custom connector and authorize through OAuth, then use list_workspaces to pick a workspace, sql_query for reads, sql_execute for card/deck authoring, get_guide for detail, next_review_card, reveal_answer, submit_review for conversational reviews, and get_usage_limits for the plan tier, its limits and this month's AI usage. Headless or CLI clients may instead send Authorization: Bearer fca_… using the agent API key from email_otp_then_api_key login (the same key as the REST agent surface), with no OAuth or browser needed.",
        authorization: {
          type: "oauth2",
