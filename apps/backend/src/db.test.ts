...
test("applyUserDatabaseScopeInExecutor sets user scope and clears workspace scope", async () => {
...
test("applyWorkspaceDatabaseScopeInExecutor sets both user and workspace scope", async () => {
  const calls: Array<Readonly<{ text: string; params: ReadonlyArray<unknown> }>> = [];
  const executor: DatabaseExecutor = {
    async query<Row extends pg.QueryResultRow>(
      text: string,
