...
extension SentryObservabilityAdapter {
    ...
    private static func foregroundOperationContext(_ observation: ForegroundOperationObservation) -> [String: Any] {
        ...
        context["review_new_count"] = observation.reviewNewCount.map { reviewNewCount in String(reviewNewCount) } ?? ""
        context["review_pending_count"] = observation.reviewPendingCount.map { reviewPendingCount in String(reviewPendingCount) } ?? ""
        context["review_total_count"] = observation.reviewTotalCount.map { reviewTotalCount in String(reviewTotalCount) } ?? ""
        context["review_filter_kind"] = observation.reviewFilterKind ?? ""
        context["review_refresh_mode"] = observation.reviewRefreshMode ?? ""
        context["review_load_kind"] = observation.reviewLoadKind ?? ""
        context["progress_summary_refresh_needed"] = observation.progressSummaryRefreshNeeded.map { progressSummaryRefreshNeeded in String(progressSummaryRefreshNeeded) } ?? ""
        context["progress_series_refresh_needed"] = observation.progressSeriesRefreshNeeded.map { progressSeriesRefreshNeeded in String(progressSeriesRefreshNeeded) } ?? ""
        context["progress_review_schedule_refresh_needed"] = observation.progressReviewScheduleRefreshNeeded.map { progressReviewScheduleRefreshNeeded in String(progressReviewScheduleRefreshNeeded) } ?? ""
        ...
        context["notification_kind"] = observation.notificationKind ?? ""
        context["notification_authorization_status"] = observation.notificationAuthorizationStatus ?? ""
        context["notification_pending_before_total_count"] = observation.notificationPendingBeforeTotalCount.map { pendingBeforeTotalCount in String(pendingBeforeTotalCount) } ?? ""
        context["notification_pending_before_review_count"] = observation.notificationPendingBeforeReviewCount.map { pendingBeforeReviewCount in String(pendingBeforeReviewCount) } ?? ""
        context["notification_pending_before_strict_count"] = observation.notificationPendingBeforeStrictCount.map { pendingBeforeStrictCount in String(pendingBeforeStrictCount) } ?? ""
        context["notification_pending_before_other_count"] = observation.notificationPendingBeforeOtherCount.map { pendingBeforeOtherCount in String(pendingBeforeOtherCount) } ?? ""
        context["notification_pending_after_total_count"] = observation.notificationPendingAfterTotalCount.map { pendingAfterTotalCount in String(pendingAfterTotalCount) } ?? ""
        context["notification_pending_after_review_count"] = observation.notificationPendingAfterReviewCount.map { pendingAfterReviewCount in String(pendingAfterReviewCount) } ?? ""
        context["notification_pending_after_strict_count"] = observation.notificationPendingAfterStrictCount.map { pendingAfterStrictCount in String(pendingAfterStrictCount) } ?? ""
        context["notification_pending_after_other_count"] = observation.notificationPendingAfterOtherCount.map { pendingAfterOtherCount in String(pendingAfterOtherCount) } ?? ""
        context["notification_delivered_before_count"] = observation.notificationDeliveredBeforeCount.map { deliveredBeforeCount in String(deliveredBeforeCount) } ?? ""
        context["notification_delivered_removed_count"] = observation.notificationDeliveredRemovedCount.map { deliveredRemovedCount in String(deliveredRemovedCount) } ?? ""
        context["notification_planned_count"] = observation.notificationPlannedCount.map { plannedCount in String(plannedCount) } ?? ""
        context["notification_attempted_count"] = observation.notificationAttemptedCount.map { attemptedCount in String(attemptedCount) } ?? ""
        context["notification_accepted_count"] = observation.notificationAcceptedCount.map { acceptedCount in String(acceptedCount) } ?? ""
        context["notification_readback_completed"] = observation.notificationReadbackCompleted.map { readbackCompleted in String(readbackCompleted) } ?? ""
