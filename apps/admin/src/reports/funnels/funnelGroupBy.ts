...
/**
 * One thing a funnel can be grouped by.
 *
 ...
 *
 * `id` is a stable URL token and never the label, so a dimension can be renamed on screen without
 * breaking links that are already shared.
 */
export type FunnelGroupByDimension = Readonly<{
  id: string;
  label: string;
...
/**
 * The key a funnel's SQL gives a person its selected dimension cannot place, as a plain literal
 * because it is produced in SQL. No dimension value can be this word: platforms, ISO country codes
 * and UI locales are all shorter, fixed shapes.
 */
 ...
 */
export function parseFunnelGroupByDimension(
  searchParams: URLSearchParams,
  funnelId: string,
  dimensions: ReadonlyArray<FunnelGroupByDimension>,
): FunnelGroupByDimension | null {
...
export function buildFunnelGroupColor(
  dimension: FunnelGroupByDimension,
 ...
 */
export function buildFunnelGroupLabel(dimension: FunnelGroupByDimension, groupKey: string): string {
...
/**
 * Both counts fold by summing, which is exact rather than an estimate: every counted person carries
 * exactly one group key, so no person is in two of the groups being added.
 */
...
export function foldFunnelGroups<StepId extends string>(
  ...
  return [
    ...
    ...(foldedGroups.length === 0 ? [] : [{
      ...
      stages: sumFunnelStages(foldedGroups.map((group) => group.stages)),
    }]),
