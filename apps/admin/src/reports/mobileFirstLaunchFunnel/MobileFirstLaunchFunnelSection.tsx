...
/** The short note shown when the step facts start after the selected range does, or `null`. */
function buildLateStepFactsNote(report: MobileFirstLaunchFunnelReport, selectedFrom: string): string | null {
  if (report.effectiveFromDate === null) {
    return "The apps had not begun reporting screen views by the end of the selected range, so no first launches are counted.";
  }

  return report.effectiveFromDate > selectedFrom
    ? `The apps began reporting screen views on ${report.effectiveFromDate}, so first launches before that day are not counted.`
    : null;
}
...
export function MobileFirstLaunchFunnelSection(props: FunnelSectionProps): JSX.Element {
  ...
  const lateStepFactsNote = report === null ? null : buildLateStepFactsNote(report, props.filters.dateRange.from);
  ...
  return (
    <section className="dashboard-section funnel-report">
      ...
      {lateStepFactsNote !== null ? <p className="report-state" aria-live="polite">{lateStepFactsNote}</p> : null}
