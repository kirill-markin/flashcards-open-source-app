...
Every chart reads `analytics.product_events_resolved` and no other product table. It touches four more relations: `org.user_settings`, joined from `actor_id` for the email the `%@example.com` exclusion needs, `auth.admin_users`, read wherever active admins are dropped, `analytics.installation_country_observations`, read for the retained connection-country samples the audience report, the country option list and the country filter all compose, and `analytics.excluded_actors`, the shared list of actors no human produced, whose active rows every filter option list and every section drops wherever the rows name an actor at all; the catalog installation funnel applies all three exclusions on each row's own `actor_id`, as [docs/admin-app.md](../../docs/admin-app.md) states. The dashboard does not join `content.review_events`, `sync.workspace_replicas`, `catalog.packages`, `community.friend_invitations`, or `community.friendships` any more.
