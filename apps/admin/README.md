...
Every chart but Study vs AI reads `analytics.product_events_resolved` and no other product table; Study vs AI reads it for the review half of each dot and `ai.chat_items` and `ai.chat_sessions` for the chat half. It touches five more relations: `org.user_settings`, joined from `actor_id` for the email the `%@example.com` exclusion needs, `auth.admin_users`, read wherever an admin grant is dropped, whether or not it was revoked, `analytics.installation_country_observations`, read for the retained connection-country samples the audience report, the country option list and the country filter all compose, `analytics.identity_links`, read to fold a chat session's raw user id onto `actor_id` so an upgraded guest is one person rather than two, and `analytics.excluded_actors`, the shared list of actors no human produced, whose active rows every filter option list and every section drops wherever the rows name an actor at all. The one exclusion rule has a fourth arm that reads no further relation, re-reading the event view itself for the actors the credential-free collector marked `automated_client IS TRUE`. Every surface applies all four arms on each row's own `actor_id`, the catalog installation funnel and the General and Audience catalog click fields included, as [docs/admin-app.md](../../docs/admin-app.md) states. The dashboard does not join `content.review_events`, `sync.workspace_replicas`, `catalog.packages`, `community.friend_invitations`, or `community.friendships` any more.
