...
`apps/android/app/src/main/generated/baselineProfiles/baseline-prof.txt` and `startup-prof.txt` are
committed artifacts. They ship inside the release AAB, so app startup and the first review are
AOT-compiled on device instead of relying only on the profiles Compose libraries bundle.

They are generated by hand and never in CI. No build and no CI job needs a device.

To regenerate:

1. Start one rootable API 37 emulator. Play Store system images cannot be rooted, so use a
   `google_apis` image: `sdkmanager --install "system-images;android-37.0;google_apis;arm64-v8a"`,
   then create an AVD from it. Stop every other emulator first and verify `adb devices` lists only
   that one device.
2. Run `./gradlew :app:generateBaselineProfile`. The plugin's own task names
   (`:app:assembleNonMinifiedRelease`, `:baselineprofile:connectedNonMinifiedReleaseAndroidTest`,
   the `benchmarkRelease` equivalents) work too, including from Android Studio's *Generate Baseline
   Profile* run configuration: the release-secret guard in `app/build.gradle.kts` deliberately
   ignores them even though their names contain "Release", because they need no signing keys, no
   `ANDROID_VERSION_CODE`, and no `ANDROID_SENTRY_DSN` and ship nothing.
3. Commit the changed profile files.

The generator lives in `apps/android/baselineprofile/`. It is a build tool, not test coverage, and
drives the real app through UiAutomator using the production Compose test tags, which
`FlashcardsApp` publishes as accessibility resource ids. It profiles the plugin-created
