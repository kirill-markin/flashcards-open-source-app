...
import com.flashcardsopensourceapp.feature.ai.input.makeAiChatAttachmentFromCameraBitmap
...
internal fun AiRouteContent(
        ...
        if (uiState.isConsentRequired) {
            ...
            Box(
                ...
                Column(
                    ...
                    AiConversationLoadErrorCard(
                        presentation = uiState.conversationErrorPresentation,
    ...
    if (activeTechnicalError == null) {
        when (val activeAlert = uiState.activeAlert) {
            ...
            is AiAlertState.AiLimitReached -> Unit
            null -> Unit
        }
    }
}
...
@Composable
private fun AiConversationLoadErrorCard(
    presentation: AiBootstrapErrorPresentation,
    modifier: Modifier
) {
    var isShowingDetails by rememberSaveable(presentation.message, presentation.technicalDetails) {
    ...
    Card(
        ...
        Column(
            ...
            if (presentation.technicalDetails.isNullOrBlank().not()) {
                TextButton(
                    ...
                    Text(
                        text = if (isShowingDetails) {
                            stringResource(id = R.string.ai_error_hide_details)
                        } else {
                            stringResource(id = R.string.ai_error_show_details)
                        }
                    )
                }
                ...
                AnimatedVisibility(visible = isShowingDetails) {
                    SelectionContainer {
                        Text(
                            text = presentation.technicalDetails.orEmpty(),
