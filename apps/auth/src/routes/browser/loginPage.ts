...
app.get("/login", async (c) => {
  ...
  // The rendered page carries this caller's own `redirect_uri` and a locale negotiated from its
  // `Accept-Language` with no `Vary` to key a cache on, so no cache between this origin and that one
  // browser may store it. Same rule as the consent page in routes/oauth/authorize.ts.
