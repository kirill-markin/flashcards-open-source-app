// This file covers the rendered page only. GET /login sets no analytics cookie: the anonymous id is
// the product domain's shared `analytics_visitor` cookie, which this service reads and never writes.
...
test("login page derives the locale from weighted Accept-Language entries when no locale hint is present", async () => {
  ...
  const response = await loginPage.request(
    ...
    {
      headers: {
        "accept-language": "am-ET;q=1.0,es-MX;q=0.6,de-DE;q=0.8",
      },
    },
  );
  const html = await readText(response);
...
test("login page falls back from an unsupported locale hint to a supported Accept-Language locale", async () => {
  ...
  const response = await loginPage.request(
    "https://auth.flashcards-open-source-app.com/login?redirect_uri=https%3A%2F%2Fapp.flashcards-open-source-app.com%2Freview&locale=am-ET",
    {
      headers: {
...
test("login page falls back safely to English when locale inputs are unsupported", async () => {
  ...
  const response = await loginPage.request(
    ...
    {
      headers: {
        "accept-language": "eu-ES,am-ET;q=0.9",
      },
    },
  );
  const html = await readText(response);
