#!/usr/bin/env bash
# Create an ACM public certificate for the admin app CloudFront domain and validate it via Cloudflare DNS.
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --admin-subdomain) ADMIN_SUBDOMAIN="$2"; shift 2 ;;
    *) echo "Unknown argument: $1" >&2; exit 1 ;;
  esac
done

if [[ -z "$DOMAIN" ]]; then
  echo "Usage: $0 --domain <domain> [--admin-subdomain <subdomain>]" >&2
  exit 1
fi
...
echo "Requesting ACM certificate for ${ADMIN_DOMAIN} in ${REGION}..."
...
CERT_ARN=$(aws acm request-certificate \
  ...
  --tags \
    ...
    Key=flashcards:purpose,Value=admin-domain \
